...
addition is the `voice_samples` table — without it, opener/reply voice anchoring
silently falls back to rules only (no error).
...
Mondays 13:00 UTC. Vercel sends a **GET** with `Authorization: Bearer <CRON_SECRET>`
